...
import {
  ...
  Image,
  StyleSheet,
  ActivityIndicator, // Loader import kiya
} from 'react-native';
...
const DryClean = ({navigation}: any) => {
  const [products, setProducts] = useState<Product[]>([]);
  const [quantities, setQuantities] = useState<{[key: string]: number}>({});
  const [loading, setLoading] = useState<boolean>(true); // Loader state add kiya
  ...
  const loadProducts = async () => {
    try {
      ...
      setLoading(false); // Loader hide kar diya jab data fetch ho gaya
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        <View style={styles.headerContainer}>
          ...
          <Text style={styles.header}>Dry Clean Services</Text>
        </View>
